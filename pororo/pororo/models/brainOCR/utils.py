...
def group_text_box(
    ...
    for poly in horizontal_list:
        ...
        if len(new_box) == 0:
            ...
            if (abs(np.mean(b_height) - poly[5]) < height_ths* 0.4 *
        ...
        if len(boxes) == 1:  # one box per line
            ...
            for box in boxes:
                if len(new_box) == 0:
                    ...
                    if abs(box[0] - x_max) < width_ths * 0.1*(
                            box[3] - box[2]):  # merge boxes # 0630) 수정 width_ths -> width_ths*0.4
                        x_max = box[1]
                        new_box.append(box)
                    else:
                        x_max = box[1]
...
# 0630 수정)x_ths,y_ths : 1.0 -> 0.01, 0.5->0.01
def get_paragraph(raw_result,
    ...
    while len([box for box in box_group if box[7] == 0]) > 0:
        ...
        # try to add group
            ...
            # 0630 수정) same_level : or -> and
            for box in box_group0:
                same_horizontal_level = (min_gx <= box[1] <= max_gx) and (min_gx <= box[2] <= max_gx)
                same_vertical_level = (min_gy <= box[3] <= max_gy) and (min_gy <= box[4] <= max_gy)
